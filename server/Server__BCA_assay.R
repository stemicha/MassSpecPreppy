...
comments_colors <- c("Error: Abs below std. curve" = "#DB4437",#red
...
BCA_OT2_template_generation<- eventReactive(input$inputButton_generate_BCA_OT2_template,{
  ...
  writeDataTable(wb = BCA_excel_output_template_OT2,
                 sheet =  1,
  ...
  for(i in 1:nrow(shiny_plate1_template)){
    ...
    addStyle(wb = BCA_excel_output_template_OT2, 
             sheet = 1,
             style = colorStyle,
             rows = i+1,
             cols = 1:5,
  ...
  BCA_OT2_protocol_out[23] <- paste(substr(x = BCA_OT2_protocol_out[23],
                                       start = 1,
  ...
  list(OT2_protocol_out = BCA_OT2_protocol_out,
...
output$dlOT2_BCA <- downloadHandler(
  ...
  content = function(fname) {
    ...
    ggsave(plot = plot_deck_layout_BCA(number_of_samples = nrow(BCA_OT2_template_generation()$OT2_template),text_color = "black"),device = "png",
    ...
    params <- list(number_of_samples = nrow(BCA_OT2_template_generation()$OT2_template),
    ...
    fs <- c(BCA_OT2_template_generation()$file_output,# OT-2 protocol
...
output$download_BCA_OT2_template<-renderUI({
  if(!is.null(BCA_OT2_template_generation()$file_output)){
    tagList(
      ...
      downloadButton(outputId = "dlOT2_BCA",
  ...
  withProgress(message = 'processing data...',
               ...
               value = 0.1,{
                 ...
                 # 96 well plate
                   ...
                   well_position_name_adding <-  c(paste(LETTERS[seq(from=1, to=8)],1,sep=""),
                 ...
                 std_conc_tibble <- tibble(Sample = c("STD160",
                                                      "STD120",
                                                      "STD80",
                                                      "STD60",
                                                      "STD40",
                 ...
                 data_samples_summary<- data_samples %>% 
                   ...
                   summarise(meanAbs = mean(as.numeric(unlist(Abs)),na.rm = T),
                 ...
                 # add hard codes standard curve -------------------------------------------
                  standard_hard_coded<- tibble(conc_mg_per_ml = c(0,0.01,0.02,0.04,0.06,0.08,0.12,0.16),
                 ...
                 standard_plot<- ggplot(std_tidy_summary,aes(meanAbs,conc_mg_per_ml))+
                   stat_smooth(data = standard_hard_coded,mapping = aes(y=conc_mg_per_ml,x=meanAbs),
                               method = "loess",
                               se=F,
                               size=5,
                               color="#2E7D32",
                 ...
                 std_tidy_cutoffs_comment<- std_tidy_summary %>% 
                   group_by(`Plate Number`) %>% 
                   summarise(std__min = meanAbs[conc_mg_per_ml==0.01],
                 ...
                 measurements_plot_hist_dens <- ggplot(data_samples_summary)+
                   ...
                   theme(strip.text = element_text(face = "bold"),
                         plot.background = element_rect(fill = "white", color = "white"))+
                   guides(color = "none")+
                   geom_vline(data = std_tidy_summary %>% 
                                group_by(`Plate Number` ) %>% 
                   ...
                   geom_vline(data = std_tidy_summary %>% 
                              ...
                              color = "red",
                              linetype="dashed")+
                   labs(title = "Histogram/Density of meanAbs of samples",
                 ...
                 scatter_conc<- ggplot(data_samples_summary_detailed)+
                   stat_smooth(aes(x=conc_µg_per_µl,
                                   y=conc_µg_per_µl_LINEAR_fit),method = "lm",color = "grey", se=F)+
                   stat_smooth(aes(x=conc_µg_per_µl,
                                   y=conc_µg_per_µl_LINEAR_fit),method = "loess",color = "black", se = F)+
                   coord_cartesian(xlim = c(0,max(max_scatter)),ylim = c(0,max(max_scatter)))+
                   theme_minimal(base_size = 18)+
                   geom_abline(slope = 1,intercept = 0, linetype = "dashed")+
                   labs(color="% outside standard curve",
                 ...
                 if(sum(data_samples_summary_detailed$outside_standard_curve)!=0){
                   scatter_conc <- scatter_conc+
                     geom_point(data = data_samples_summary_detailed %>% filter(outside_standard_curve==F),
                                mapping = aes(x=conc_µg_per_µl_wo_dil_factor*dilution_factor,
                                              y=conc_µg_per_µl_wo_dil_factor_LINEAR_fit*dilution_factor,
                                              shape=outside_standard_curve,
                                              color=comment),
                                size=5,alpha=0.5)+
                     geom_point(data = data_samples_summary_detailed %>% filter(outside_standard_curve==T),
                                mapping = aes(x=conc_µg_per_µl_wo_dil_factor*dilution_factor,
                                              y=conc_µg_per_µl_wo_dil_factor_LINEAR_fit*dilution_factor,
                                              shape=outside_standard_curve,
                                              color=comment),
                     ...
                     geom_text_repel(data = data_samples_summary_detailed %>% 
                                       filter(outside_standard_curve==T) %>% 
                                       top_n(outside_standard_curve_percentage,n = 10),
                                     mapping = aes(x=conc_µg_per_µl_wo_dil_factor*dilution_factor,
                                                   y=conc_µg_per_µl_wo_dil_factor_LINEAR_fit*dilution_factor,
                                                   label = Sample),min.segment.length = 0.001)
                 }else{
                   scatter_conc <- scatter_conc+
                     geom_point(aes(x=conc_µg_per_µl_wo_dil_factor*dilution_factor,
                                    y=conc_µg_per_µl_wo_dil_factor_LINEAR_fit*dilution_factor,
                                    shape=outside_standard_curve,
                                    color=comment),
                                size=5,
                 ...
                 comment_count_barchart <- ggplot(data = comment_count,
                                                  mapping = aes(x = comment,
                                                                y = n,
                                                                fill = comment))+
                   geom_bar(stat = "identity")+
                 ...
                 list(standard_plot = standard_plot/std_cv_plot/standard_res_plot/standard_zscore_plot,
                      residuals_tidy = residuals_tidy,
...
output$data_samples_summary_out<-DT::renderDataTable({
  ...
  DT::datatable(
    ...
    DT::formatStyle('comment',
                    backgroundColor = styleEqual(c(names(comments_colors)),
                                                 c(comments_colors))
    ) %>% 
...
output$data_cv_plot_out_ui<- renderUI({
  withSpinner(ui_element = plotOutput("data_cv_plot_out",height = if_else(calculations()$dilutions==1,500,200*calculations()$dilutions)),
...
output$standard_plot_out_ui<- renderUI({
  withSpinner(ui_element = plotOutput("standard_plot_out",height = 400*4,width = if_else(calculations()$plates==1,700,600*calculations()$plates)),
              type = getOption("spinner.type", default = 8),
              proxy.height = "200px",
              color = getOption("spinner.color", default = "#813D9C")
  )
...
output$measurements_plot_out_ui<- renderUI({
  withSpinner(ui_element = plotOutput("measurements_plot_out",width = if_else(calculations()$dilutions==1,700,600*calculations()$dilutions),height = ifelse(calculations()$plates==1,1200,200*calculations()$plates*3)),
              type = getOption("spinner.type", default = 8),
              proxy.height = "200px",
              color = getOption("spinner.color", default = "#813D9C")
  )
...
output$comment_count_barchart_out_ui<- renderUI({
  withSpinner(ui_element = plotOutput("comment_count_barchart_out",height = ifelse(calculations()$plates==1,600,100*calculations()$plates*3)),
              type = getOption("spinner.type", default = 8),
              proxy.height = "200px",
              color = getOption("spinner.color", default = "#813D9C")
  )
...
output$BCA_download_analysis<-renderUI({
  if(!is.null(calculations()$data_samples_summary_detailed)){
    tagList(
      ...
      downloadButton(outputId = "BCA_downloadProcessedData",
...
output$BCA_downloadProcessedData <- downloadHandler(
  ...
  content = function(fname) {
    ...
    withProgress(message = 'generate download...',
                 style = "notification",
                 value = 0.1,{
                   ...
                   writeDataTable(wb = excel_output,
                   ...
                   fs <- c(paste(file_name(),"__shinyBCA_output.xlsx",sep=""),
